...
module.exports = async (GuaranteeClaimEventList, accountId) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Prepare Guarantee Events
      if (GuaranteeClaimEventList !== undefined) {
        GuaranteeClaimEventList.map(async (event) => {
          try {
            const {
            ...
            const exists = await GuaranteeClaimEvent.findOne({
              where: { amazonOrderId, postedDate },
            });
            if (exists == null) {
              await GuaranteeClaimEvent.create(
                {
                  postedDate,
                  amazonOrderId,
                  accountId,
            ...
            console.log(
              'Error on creating guarantee claim events with association'
            );
            console.log(error);
          }
        });
      }
      return resolve(GuaranteeClaimEventList);
    } catch (error) {
      return reject(
        new Error(
          `Something went wrong on saving guarantee claim events. ${error.message}`
        )
      );
    }
  });
