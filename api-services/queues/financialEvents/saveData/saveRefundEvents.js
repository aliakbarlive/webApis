...
module.exports = async (RefundEventList, accountId) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Prepare Refund Events
      if (RefundEventList !== undefined) {
        RefundEventList.map(async (event) => {
          try {
            const {
            ...
            const exists = await RefundEvent.findOne({
              where: { amazonOrderId, postedDate },
            });
            if (exists == null) {
              await RefundEvent.create(
                {
                  postedDate,
                  amazonOrderId,
                  accountId,
            ...
            console.log('Error on creating refund events with association');
            console.log(error);
          }
        });
      }
      return resolve(RefundEventList);
    } catch (error) {
      return reject(
        new Error(
          `Something went wrong on saving refund events. ${error.message}`
        )
      );
    }
  });
