...
const MetricsFilter = ({
  ...
  ordersAttribute = 'attributedUnitsOrdered30d',
}) => {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const [metrics, setMetrics] = useState([
    ...additionalMetric,
    { display: t('Advertising.Metrics.ACOS'), key: 'acos', min: '', max: '' },
    {
      display: t('Advertising.Metrics.Impr'),
      key: 'impressions',
      min: '',
      max: '',
    },
    {
      display: t('Advertising.Metrics.Clicks'),
      key: 'clicks',
      min: '',
      max: '',
    },
    { display: t('Advertising.Metrics.Spend'), key: 'cost', min: '', max: '' },
    {
      display: t('Advertising.Metrics.Sales'),
      key: salesAttribute,
      min: '',
      max: '',
    },
    {
      display: t('Advertising.Metrics.Orders'),
      key: ordersAttribute,
      min: '',
      max: '',
    },
    { display: t('Advertising.Metrics.CPC'), key: 'cpc', min: '', max: '' },
    { display: t('Advertising.Metrics.CTR'), key: 'ctr', min: '', max: '' },
    { display: t('Advertising.Metrics.CR'), key: 'cr', min: '', max: '' },
  ]);
    ...
    <Menu as="div" className="relative inline-block text-left">
      <>
        ...
        >
          <Menu.Items
            static
            className="z-50 origin-top-right absolute right-0 mt-2 w-72 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none"
          >
            <div className="px-4 py-3">
                  ...
                  <div key={index} className="flex mb-1">
                    ...
                    <div className="flex block">
                      <input
                        id={`${index}-min`}
                        type="number"
                        placeholder={t('Advertising.MetricsFilter.Min')}
                        value={metric.min}
                      ...
                      <input
                        id={`${index}-max`}
                        type="number"
                        placeholder={t('Advertising.MetricsFilter.Max')}
                        value={metric.max}
                        onChange={updateValue}
              ...
              <div className="mt-4">
                <button
                  ...
                  onClick={applyFilter}
                >
                  {t('Advertising.Common.Filter.Apply')}
                </button>
                <button
                ...
                >
                  {t('Advertising.Common.Filter.Cancel')}
                </button>
              </div>
            </div>
          </Menu.Items>
