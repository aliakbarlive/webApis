...
const BiggestChangesByProduct = ({
  ...
  useEffect(() => {
    axios
      .get('/ppc/analytics/product-changes', {
        params: {
          ...params,
          accountId,
          marketplace,
      ...
      .then((response) => {
        ...
        data.rows = data.rows.map((row) => {
          row.name = row.asin ?? row.sku;
          if (row.listing.title) {
            row.name = row.listing.asin + ' - ' + row.listing.title;
          }
          return row;
        });
        setProducts(data);
    ...
    <div className="relative border border-gray-300 bg-white rounded-md shadow-md">
      <div className="flex justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-6 rounded-t-md">
        <p className="text-sm leading-6 font-medium text-gray-700">
          Biggest Changes By ASIN (Products)
        </p>
        <select
          value={attribute}
          onChange={(e) => setAttribute(e.target.value)}
          className="block pl-3 pr-10 py-2 border-gray-300 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm rounded-md disabled:bg-gray-100 text-sm text-gray-700  w-48"
        >
          {metrics.map((option) => {
            return (
      ...
      <div className="m-4 mb-16">
        <ResponsiveContainer
          width={'100%'}
          height={50 * products.rows.length}
          debounce={50}
        >
          <BarChart
            data={products.rows}
          ...
          >
            ...
            <YAxis
              ...
              tickFormatter={(value, index) => {
                const product = products.rows[index];
                const metric = metrics.find((m) => m.value === attribute);
                let formattedValue = metric.formatter(value);

                if (product.previous) {
                  const percentage = value / Math.abs(product.previous);
...
export default BiggestChangesByProduct;
