...
const Snapshots = ({ account, marketplace }) => {
  ...
  const loadSnapshots = useCallback(() => {
    snapshots.map((item) => {
      let snapshot = item;
      dispatch(getSnapshotAsync(snapshot.value, account, marketplace)).then(
        (data) => {
          snapshot.data = data;
  ...
  }, [dispatch, snapshots, setSnapshotDetails, account, marketplace]);
