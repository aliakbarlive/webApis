...
const MultipleFilter = ({
  ...
  return (
    <Menu as="div" className={`relative inline-block text-left ${className}`}>
      <div>
        {showLabel && (
          <p className="block text-sm font-medium text-gray-700 pb-1">
            {t('MultipleFilter.Label')}
          </p>
        )}
        <Menu.Button className="inline-flex justify-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none">
          {selectedCount
            ? `${selectedCount} ${
                hasSelectedPreview ?? t('MultipleFilter.Preview')
              }`
            : noSelectedPreview ?? t('MultipleFilter.NoSelected')}
          <ChevronDownIcon
      ...
      >
        <Menu.Items
          className={`${menuClass} z-50 absolute right-0 w-56 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none`}
        >
          <ul>
                      ...
                      <Checkbox
                        ...
                        checked={
                          optionKey in selected
                            ? selected[optionKey].includes(value.toString())
                            : false
                        }
                      />
                    </li>
