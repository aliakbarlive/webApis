...
import { selectSidebarCollapse } from './layout/layoutSlice';
...
const Sidebar = ({ location, sidebar, layout }) => {
  const sidebarCollapse = useSelector(selectSidebarCollapse);
  ...
  return (
    <nav
      className={'sidebar sidebar-sticky' + (sidebarCollapse ? ' toggled' : '')}
    >
      <div className="sidebar-content">
        <PerfectScrollbar>
          <a className="sidebar-brand" href="/">
            <Box className="align-middle text-primary" size={24} />{' '}
            <span className="align-middle">BetterSeller</span>
          </a>

          <ul className="sidebar-nav">
            {routes
              .filter((route) => authorize(authenticatedUser, route.roles))
              .map((category, index) => {
                return (
