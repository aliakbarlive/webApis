...
const Subscription = ({ history }) => {
  ...
  useEffect(() => {
    if (country && country.name === 'Canada' && province) {
  ...
  const createSubscription = async (values) => {
    ...
    try {
      ...
      dispatch(
        setAppNotification('success', t('NewSubscription.HostedPageRedirect'))
      );
                  ...
                  <select
                    ...
                    className={classNames(
                      formik.touched.country && formik.errors.country
                        ? 'border-red-300'
                        : 'border-gray-300',
                      'block w-full  rounded-md shadow-sm focus:ring-green-200 focus:border-green-300 sm:text-sm'
                    )}
