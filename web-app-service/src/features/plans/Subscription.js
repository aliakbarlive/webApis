...
const Subscription = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`/accounts/${accountId}/subscription`).then((res) => {
      ...
      <div className="sm:grid sm:grid-cols-3 sm:gap-4">
        <div className="sm:col-span-1">
          ...
          {row(
            ...
            <Badge
              color={classnames({
                green: subscription.status === 'live',
                gray: subscription.status === 'non_renewing',
                yellow: subscription.status === 'cancelled',
                red: subscription.status === 'expired',
                blue: subscription.status === 'paused',
              })}
              classes="uppercase"
              rounded="md"
            >
