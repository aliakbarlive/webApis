...
const AdvApiAuthorization = ({ location, isAuthorizing, setActiveStep }) => {
  ...
  useEffect(() => {
    if (location.search.includes('cpc_advertising')) {
      dispatch(authorizeAdvApiAsync(qs.parse(location.search.slice(1))));
  ...
  return (
    <>
      ...
      <div className="mt-8 p-8 text-center border border-gray-200 border-dotted rounded-lg">
        ...
        <div className="mt-6">
          <a
            href={`https://www.amazon.com/ap/oa?client_id=${process.env.REACT_APP_ADV_API_CLIENT_ID}&scope=cpc_advertising:campaign_management&response_type=code&redirect_uri=${process.env.REACT_APP_CLIENT_ADV_API_REDIRECT_URL}`}
